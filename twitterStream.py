...
def make_plot(counts):
    ...
    for count in counts:
        for word in count:
            if word[0] == "positive":
                pos_count.append(word[1])
            else:
                neg_count.append(word[1])

    plt.axis([-1, len(pos_count), 0, max(max(pos_count),max(neg_count))+110])
    pos = plt.plot(pos_count, 'b-', marker = 'o', markersize = 10)
    neg = plt.plot(neg_count, 'g-', marker = 'o', markersize = 10)
    plt.legend((pos,neg),('Positive','Negative'),loc=2)
    plt.xticks(np.arange(0, len(pos_count), 1))
...
def stream(ssc, pwords, nwords, duration):
    ...
    wordPair = tweet_words.map(lambda word: ('positive', 1) if (word in pwords) else ('negative', 1))

    # Count and print the number of postive and negative words
    wordCounts = wordPair.reduceByKey(lambda x, y: x + y)
    totalCount = wordPair.updateStateByKey(updateFunction)
    totalCount.pprint()
